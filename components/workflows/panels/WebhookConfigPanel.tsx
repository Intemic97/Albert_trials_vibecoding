...
import { Copy, Globe, X } from '@phosphor-icons/react';
...
export const WebhookConfigPanel: React.FC<WebhookConfigPanelProps> = ({ nodeId, node, onSave, onClose, showToast, generateId, API_BASE }) => {
  const [webhookUrl, setWebhookUrl] = useState(node?.config?.webhookUrl || '');
  const [webhookToken, setWebhookToken] = useState(node?.config?.webhookToken || '');
